...
// El conteo se DERIVA del catálogo, no se escribe a mano: el ciclo de precios
// por forma de pago sumó el séptimo permiso (`PLANES_PAGO`) y dejó estos casos
// en rojo con un "2 de 6" que ya no existía. Misma regla que este repo aplica
// para el número de pantallas o el `version` de package.json — un número en dos
// lugares es un número que se desincroniza.
const CONTEO = `2 de ${CLAVES_DE_PERMISO.length} permisos`
...
describe('CardEquipo: el patrón grid + display:contents (Task 10)', () => {
  ...
  describe('la columna "Permisos"', () => {
    ...
    it('en el teléfono el disparador se funde en la línea de acciones, sin duplicar el nodo', async () => {
      ...
      expect(html.split(CONTEO)).toHaveLength(2)
      ...
      expect(html.indexOf(CONTEO)).toBeGreaterThan(linea)
      expect(html.indexOf('Reactivar')).toBeGreaterThan(html.indexOf(CONTEO))
    })
    ...
    it('en el teléfono el disparador paga el mismo tratamiento que los links de acciones (10px), y en escritorio el botón de siempre', async () => {
      const html = await render()
      const desde = html.lastIndexOf('<button', html.indexOf(CONTEO))
