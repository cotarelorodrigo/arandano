...
describe('Detalle: "Qué se vendió" — el patrón grid + display:contents', () => {
  ...
  it('hay 4 role="columnheader": Artículo, Cantidad, Precio, Subtotal', () => {
    const html = renderDetalle()
    expect(html.match(/role="columnheader"/g)).toHaveLength(4 + 5) // + las 5 de "Cómo se pagó"
  ...
  it('cada fila de ítem lleva lg:contents, role="row" y 4 celdas role="cell"', () => {
    ...
    // 4 celdas por ítem × 2 ítems, más las 5 de "Cómo se pagó" (1 pago).
    expect(html.match(/role="cell"/g)).toHaveLength(4 * 2 + 5)
  })
...
describe('Detalle: "Cómo se pagó" — el patrón grid + display:contents', () => {
  it('el contenedor es la tabla ARIA: 1 columna en el teléfono, 5 en escritorio', () => {
    const html = renderDetalle()
    expect(html).toMatch(/class="[^"]*\bgrid-cols-1\b[^"]*\blg:grid-cols-\[1fr_110px_130px_130px_140px\]/)
  })

  it('hay 5 role="columnheader" para esta tabla: Medio, Moneda, Cotización, Monto, En pesos', () => {
    const html = renderDetalle()
